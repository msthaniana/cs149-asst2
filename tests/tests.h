...
//TODO: Need to make this better rather than just being a copy of the other taskṣ
/*
 * ==================================================================
...
TestResults yourTest(ITaskSystem* t, bool do_async) {
    ...
    SimpleMultiplyTask first = SimpleMultiplyTask(num_elements, output);
    SimpleMultiplyTask second = SimpleMultiplyTask(num_elements, output);
    ...
    if (do_async) {
        ...
        t->runAsyncWithDeps(&first, num_tasks, emptyDeps);
        t->runAsyncWithDeps(&second, num_tasks, emptyDeps);
        t->sync();
    } else {
    ...
    for (int i=0; i<num_elements; i++) {
        int value = i + 1; // TODO: initialize value
